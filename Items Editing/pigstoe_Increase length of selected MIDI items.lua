-- Description: Increase length of selected MIDI items
-- Version: 1.0.2
-- Author: pigstoe
...
function Main()
  ...
  for i = 0, count_si - 1 do
    ...
    if math.fmod(pos0_e, len_unit) == 0 then
      reaper.MIDI_SetItemExtents(item, pos0_s, pos0_e + len_unit)
    else
      local mul = math.floor(pos0_e / len_unit) + 1
      reaper.MIDI_SetItemExtents(item, pos0_s, len_unit * mul)
    end
  end
  reaper.Undo_EndBlock("Increase length of selected MIDI items", 4)
end
